...
response.setContentType("text/html");			
 ...
 pw.println("<body>");
		...
		pw.print("<td><div id='shop_item'>");
			...
			String categoryForCart = "";
			String dataGetType = data.getType();
			if(dataGetType.equals("fitnessWatches") || dataGetType.equals("smartWatches")) {
				categoryForCart = dataGetType.substring(0, dataGetType.length()-2);
			} else if(dataGetType.equals("vrMap")) {
				categoryForCart = "virtualReality";
			} else if(dataGetType.equals("accessorie")){
				categoryForCart = "accessories";
			}else {
				categoryForCart = dataGetType.substring(0, dataGetType.length()-1);
			}
			pw.print("<li><form method='post' action='Cart'>" +
					"<input type='hidden' name='name' value='"+data.getId()+"'>"+
					"<input type='hidden' name='type' value='"+categoryForCart+"'>"+
